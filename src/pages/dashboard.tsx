...
import { ErrorLogListPage as ShowCaseList } from 'pages/error-logs';
...
function DashboardPage() {
  ...
  const errorLogStat = useSWR<PaginatedInterface<ErrorLogInterface>>(
    () => `/error-logs`,
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Error Logs',
      result: errorLogStat?.data?.totalCount,
      isLoading: errorLogStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`error-logs`} passHref legacyBehavior>
                  <Button
